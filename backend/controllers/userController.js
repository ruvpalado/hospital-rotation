...
/**
 * Readable-ish temporary password (e.g. "Rt8k-Qm2p") for CSV-provisioned
 * doctor accounts. Excludes visually ambiguous characters (0/O, 1/l/I).
 * Only ever appears in plaintext in the one-time welcome email -- never
 * logged, never returned in the API response.
 */
...
/**
 * Developer-only (see routes/users.js requireDeveloperEmail): bulk-creates
 * Physician accounts from an uploaded CSV. Expected columns (header row
 * required): full_name, email, phone (optional), site_code (optional,
 * matches Site.short_code), department_code (optional, matches
 * Department.code).
 *
 * This is intentionally row-tolerant rather than all-or-nothing: a bad row
 * (missing field, duplicate email, unknown site/department code) is skipped
 * with a reason and the rest of the file still processes. Every successfully
 * created account gets a random temporary password, emailed once (same
 * "email" notification channel as the rest of the app) -- the recipient is
 * expected to use Forgot Password to set their own password on first login.
 */
exports.bulkUploadDoctors = async (req, res) => {
  try {
    ...
    const physicianRole = await Role.findOne({ where: { key: 'physician' } });
    if (!physicianRole) {
      return res.status(500).json({ error: "'physician' role not found -- run the seed script first." });
    }
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      const siteCode = (row.site_code || row.siteCode || '').trim();
      const departmentCode = (row.department_code || row.departmentCode || '').trim();

      if (!fullName || !email) {
        skipped.push({ row: rowNumber, email: email || null, reason: 'Missing required full_name or email' });
        continue;
      }

      const existing = await User.findOne({ where: { email } });
      if (existing) {
        skipped.push({ row: rowNumber, email, reason: 'Email already registered' });
        continue;
      }

      let site = null;
      if (siteCode) {
        site = await Site.findOne({ where: { short_code: siteCode } });
        if (!site) {
          skipped.push({ row: rowNumber, email, reason: `Unknown site_code "${siteCode}"` });
          continue;
        }
      }
      ...
      let department = null;
      if (departmentCode) {
        department = await Department.findOne({ where: { code: departmentCode } });
        if (!department) {
          skipped.push({ row: rowNumber, email, reason: `Unknown department_code "${departmentCode}"` });
          continue;
        }
      }
      ...
      const user = await User.create({
        ...
        home_site_id: site ? site.id : null,
        home_department_id: department ? department.id : null,
        language_pref: 'en',
        // Provisioned directly by the developer, not a self-registration --
        // skips the pending-approval flow entirely, same as seed.js accounts.
      ...
      try {
        await sendNotification({
          userId: user.id,
          channel: 'email',
          title: 'Your OBGYN Master Rotation Account',
          message: `Hi ${fullName}, an account has been created for you on OBGYN Master Rotation.\n\nEmail: ${email}\nTemporary password: ${tempPassword}\n\nPlease log in and use "Forgot Password" on the login page to set your own password.`,
          email: user.email,
        });
      } catch (notifyErr) {
        console.error(`Failed to send welcome email to ${email}:`, notifyErr.message);
      }

      created.push({ row: rowNumber, id: user.id, fullName, email });
    }

    await AuditLog.create({
      user_id: req.user.id,
      action: 'create',
      entity_type: 'bulk_doctor_upload',
      details: { createdCount: created.length, skippedCount: skipped.length },
    });

    res.json({
      message: `${created.length} doctor account(s) created, ${skipped.length} row(s) skipped.`,
      created,
      skipped,
    });
  } catch (err) {
