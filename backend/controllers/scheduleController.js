...
/**
 * "+ Add Next Block": create the next sequential rotation for a physician.
 * Given a physician (by physician_id for a registered account, or by
 * physician_name for a free-typed one), finds their highest existing block
 * number, and creates a new schedule for block N+1 -- reusing the same
 * site/department as their latest rotation (facility stays consistent) and
 * that next block's own dates.
 */
exports.addNextBlock = async (req, res) => {
  try {
    ...
    // Existing rotations for this physician, newest block first, so [0] is
    // both the highest block number and the reference for site/department.
    const existing = await RotationAssignment.findAll({
      where: physicianWhere,
      include: [{ model: Block }],
      order: [[Block, 'block_number', 'DESC']],
    });
    if (existing.length === 0) {
      return res.status(400).json({ error: 'This physician has no schedule yet -- create their first block from + Add Schedule.' });
    }

    const reference = existing[0];
    const highestBlockNumber = reference.Block.block_number;
    const nextBlockNumber = highestBlockNumber + 1;

    const nextBlock = await Block.findOne({ where: { block_number: nextBlockNumber } });
    if (!nextBlock) {
      return res.status(400).json({ error: `No further block after Block ${highestBlockNumber} -- the curriculum ends there.` });
    }

    // Duplicate guard (should never trigger since we take highest+1, but the
    // one-schedule-per-physician-per-block rule is enforced everywhere).
    const dup = existing.some((a) => a.Block.block_number === nextBlockNumber);
    if (dup) {
      return res.status(409).json({ error: `This physician already has a schedule for Block ${nextBlockNumber}.` });
    }

    const assignment = await RotationAssignment.create({
      physician_id: reference.physician_id,
      physician_name: reference.physician_name,
      site_department_id: reference.site_department_id, // same facility/department
      block_id: nextBlock.id,
      start_date: nextBlock.start_date,
      end_date: nextBlock.end_date,
      status: 'scheduled',
    });

    // Auto-create the weekly attendance rows aligned to the block's dates.
    const start = new Date(nextBlock.start_date);
    for (let i = 0; i < nextBlock.total_weeks; i++) {
      const weekStart = new Date(start);
      weekStart.setDate(weekStart.getDate() + i * 7);
      await RotationWeek.create({
        rotation_assignment_id: assignment.id,
    ...
    console.error(err);
    res.status(500).json({ error: 'Failed to add next block', details: err.message });
  }
};
